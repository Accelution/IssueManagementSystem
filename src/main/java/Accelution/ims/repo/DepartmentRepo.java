...
//    Iterable<Department> findByStatus(@Param("userId") Integer userId);
//    @Query("SELECT p.`id`,(SELECT `name` FROM `hris_new`.`department` WHERE `id` = p.`subtype`) AS `subtype`,p.`name`,p.`link`,p.`status` FROM `forms` p WHERE `status`='active'")
    Iterable<Departments> findByStatus(String status);

    @Query("SELECT `id` AS `value`, `name` AS `text` FROM `department`")
